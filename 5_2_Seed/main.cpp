...
struct Almanac
{
    ...
    vector<Seed> seeds;
    CustomMap seedsMap = CustomMap(SEEDS_TOKEN);
    ...
    CustomMap & getCustomMap(const string &token)
    {
        ...
        else if (HUMIDITYTOLOCATION_TOKEN == token)
        {
...
void parseSeeds(const string &line, Almanac &almanac)
{
    ...
    for (size_t i=0; i<numbers.size();)
    {
        ...
        for (long j=0; j<stol(numbers.at(i+1)); j++)
        {
            // cout << "seed: " << stol(numbers.at(i)) + j << endl;
            almanac.seeds.emplace_back(static_cast<long>(stol(numbers.at(i)) + j));
        }
        i += 2;
    }
}

void parseSeedsMap(const string &line, Almanac &almanac)
{
    if (line.find(Almanac::SEEDS_TOKEN) == std::string::npos)
    {
        cout << "No seeds!" << endl;
    ...
    for (size_t i=0; i<numbers.size();)
    {
        ...
        cout << "seed start: " << stol(numbers.at(i)) << " length: " << stol(numbers.at(i+1)) << endl;
        almanac.seedsMap.sourceRangeStart.push_back(stol(numbers.at(i)));
        almanac.seedsMap.destinationRangeStart.push_back(stol(numbers.at(i)));
        almanac.seedsMap.rangeLength.push_back(stol(numbers.at(i+1)));
        i += 2;
    }
}
...
void parseAlmanac(const vector<string> &lines, Almanac &almanac)
{
    ...
    parseSeedsMap(lines.at(idx), almanac);
...
void getLocationFromSeedRange(const size_t id, Almanac &almanac)
{
    long minLocation = LONG_MAX;
    cout << "Begin of " << id+1 << "/" << almanac.seedsMap.sourceRangeStart.size() << endl;
    for (size_t j=0; j<almanac.seedsMap.rangeLength.at(id); j++)
    {
        long value = almanac.seedsMap.sourceRangeStart.at(id) + j;
        // cout << value << " ";
        value = getMappingValue(almanac.SEEDTOSOIL_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.SOILTOFERTILIZER_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.FERTILIZERTOWATER_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.WATERTOLIGHT_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.LIGHTTOTEMPERATURE_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.TEMPERATURETOHUMIDITY_TOKEN, value, almanac);
        // cout << value << " ";
        value = getMappingValue(almanac.HUMIDITYTOLOCATION_TOKEN, value, almanac);
        // cout << value << endl;
    ...
    cout << "Min location from: " << id+1 << "/" << almanac.seedsMap.sourceRangeStart.size() << 
            ": " << minLocation << endl;
    // return minLocation;
}

void parseSeedsMap(Almanac &almanac)
{
    vector<std::thread> threads{};
    for (size_t i=0; i<almanac.seedsMap.sourceRangeStart.size(); i++)
    {
        threads.push_back(std::thread(getLocationFromSeedRange, i, std::ref(almanac)));
        // getLocationFromSeedRange(i, almanac);
    }
...
int main()
{
    ...
    // string filename("example_input"); // 46
