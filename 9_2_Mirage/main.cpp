...
struct Oasis
{
    ...
    void reversedExtrapolateData()
    {
        for (auto &measuredDataHistory : data)
        {
            for (int i=measuredDataHistory.size()-1; i>=0; i--)
            {
                if ((measuredDataHistory.size()-1) == i)
                {
                    measuredDataHistory.at(i).push_front(0);
                    continue;
                }
                measuredDataHistory.at(i).push_front(measuredDataHistory.at(i).at(0) - 
                                                    measuredDataHistory.at(i+1).at(0));
            }
        }
    }
    ...
    long getSumOfFirstExtrapolateValues()
    {
        return std::accumulate(data.cbegin(), data.cend(), 0, [] (long sum, const MeasuredDataHistory &measuredDataHistory)
        {
            return sum + measuredDataHistory.at(0).at(0);
        });
...
int main()
{
    ...
    // string filename(cwd.parent_path()/"example_input"); // 2
    ...
    oasis.reversedExtrapolateData();
    cout << endl << "After reversed extrapolate" << endl;
    cout << oasis << endl;
    cout << "getSumOfFirstExtrapolateValues: " << oasis.getSumOfFirstExtrapolateValues() << endl;
