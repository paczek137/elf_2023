...
void parseCards(const vector<string> &lines, vector<Card> &cards)
{
    ...
    for (size_t i=0; i<lines.size(); i++)
    {
        ...
        card.copies = 1;
        int currentCardCopies = wonCopies.size() ? wonCopies.at(0) + 1 : 1;
        cout << "currentCardCopies: " << currentCardCopies << endl;
        if (not wonCopies.empty())
        {
            card.copies += wonCopies.at(0);
            wonCopies.pop_front();
        }
        cout << "copies: " << card.copies << endl;
        ...
        if (not v.empty())
        {
            ...
            for (size_t i=0; i<v.size(); i++)
            {
                if (i < wonCopies.size())
                {
                    wonCopies.at(i) += currentCardCopies;
                }
                else
                {
                    wonCopies.push_back(currentCardCopies);
                }
            }
        }
...
int main()
{
    ...
    // string filename("example_input"); // 30
    ...
    cout << "copies: " << std::accumulate(cards.cbegin(), cards.cend(), 0, [] (int sum, const Card &card)
    {
